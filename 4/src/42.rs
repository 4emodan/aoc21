...
fn main() -> std::io::Result<()> {
    ...
    'nums: for num in numbers {
        ...
        for (board_idx, col, row) in bonds.unwrap() {
            ...
            *mask = *mask | (1u32 << idx);
            ...
            if is_win > 0 {
                println!("Board {} won at number {}!", board_idx, num);
                board.win_num = Some((col, row));
    ...
    println!("The last winner is Board{} at number {}!", win_board.unwrap(), win_num.unwrap());
    print_board(winner);
...
struct Board {
    values: Vec<u8>,
    mark_mask: u32,
    win_num: Option<(u8, u8)>,
}

impl Board {
    fn has_won(&self) -> bool {
        return self.win_num.is_some();
    }
}